...
def inference(images, labels=[], is_training=None, keep_prob=tf.constant(1.0)):
    ...
    conv2 = ops.conv2d_in('conv2', conv1, [4,4,64,128], stride=[1,2,2,1]) # 56x56
    conv3 = ops.conv2d_in('conv3', conv2, [4,4,128,256], stride=[1,2,2,1]) # 28x28
    conv4 = ops.conv2d_in('conv4', conv3, [4,4,256,512], stride=[1,2,2,1]) # 14x14
    conv5 = ops.conv2d_in('conv5', conv4, [4,4,512,512], stride=[1,2,2,1]) # 7x7
    conv6 = ops.conv2d_in('conv6', conv5, [4,4,512,512], stride=[1,2,2,1]) # 4x4
    conv7 = ops.conv2d_in('conv7', conv6, [4,4,512,512], stride=[1,2,2,1]) # 2x2
    conv8 = ops.conv2d_in('conv8', conv7, [4,4,512,512], stride=[1,2,2,1]) # 1x1

    # upsample
    up1_1 = tf.image.resize_nearest_neighbor(conv8, [2,2], name='up1') # 2x2
    up1 = ops.conv2d_in('upc1', up1_1, [4,4,512,512])
    ...
    up2 = ops.conv2d_in('upc2', up2_1, [4,4,1024,512])
